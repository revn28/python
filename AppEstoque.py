...
class GerenciadorEstoque:
    ...
    def atualizar_produto(self, id, nome, descricao, quantidade, preco):
        self.cursor.execute('SELECT COUNT(*) FROM produtos WHERE id=?', (id,))
        produto_existente = self.cursor.fetchone()[0]
        if not produto_existente:
            print("ID do produto não encontrado. Retornando ao menu principal.")
            return
    ...
    def registrar_venda_usuario(self):
            ...
            produto_existente = gerenciador.buscar_produto_por_id(id_produto)
